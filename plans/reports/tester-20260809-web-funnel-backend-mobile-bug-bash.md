...
Status: FIX IMPLEMENTED LOCALLY — web quiz, sandbox checkout, RevenueCat email delivery, and Firebase email delivery passed. Staging retest is still required after deploying the web/mobile changes.
...
- Backend: staging Render service; health endpoint returned HTTP 200 and OpenAPI exposed the lead, preflight, finalize, and user-sync routes.
...
1. Completed the Vietnamese quiz and reached the generated plan and checkout.
2. Paddle sandbox accepted the test payment and showed bank-processing, followed by the post-payment 75% return offer.
3. RevenueCat delivered `[Sandbox] Redeem your Nutree subscription` to the test Gmail inbox.
4. The mobile app opened the RevenueCat link and displayed its passwordless email sign-in screen.
5. The app sent the Firebase sign-in email. Gmail received the first link at 17:32 UTC and a fresh resend at 17:36 UTC.
6. The staging app reached Firebase and RevenueCat startup services successfully; native logs showed Firebase and RevenueCat responses, including a 200 response for the fresh Firebase email-link sign-in attempt.
...
Fix implemented: `/postcheckout` now resumes the persisted anonymous provider ID plus link digest and retries correlation for up to eight attempts without persisting the redemption URL. A successful acknowledgement clears the pending digest; exhaustion gives an explicit refresh/recovery message.
...
Retest gate: deploy/rebuild the mobile fix, use one fresh Firebase link exactly once on a clean staging app state, and assert the three backend routes plus Home/subscription state.
...
- After the fixes are deployed, does a fresh one-time Firebase link produce preflight, finalize, user sync, and Home on the same mobile session?
