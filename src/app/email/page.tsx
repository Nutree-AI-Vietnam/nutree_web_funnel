...
const TRUST_AVATAR_IMAGES = [
  '/images/trust-female.webp',
  '/images/trust-aiony.webp',
  '/images/trust-good-faces.webp',
  '/images/trust-ali.webp',
  '/images/trust-jurica.webp',
] as const;
    ...
    <ConversionShell className="justify-center">
      <section className="rounded-[1.5rem] border border-border-brand bg-white/92 p-5 shadow-[0_18px_55px_rgb(16_39_32_/_0.08)]">
        <div className="flex flex-col gap-4">
          <div className="flex items-center gap-3 rounded-2xl bg-emerald-50 px-4 py-3 text-sm font-extrabold text-emerald-800">
            <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-emerald-500 text-white">✓</span>
            <span>{vi.email.readyBadge}</span>
          </div>
          ...
          <div className="border-t border-border-brand/70 pt-4 text-center">
            <div className="flex justify-center -space-x-3" aria-hidden="true">
              {TRUST_AVATAR_IMAGES.map((src) => (
                <span key={src} className="relative h-12 w-12 overflow-hidden rounded-full border-2 border-white bg-mist shadow-sm">
                  <Image src={src} alt="" fill sizes="48px" className="object-cover" />
                </span>
              ))}
            </div>
            <p className="mt-3 text-sm font-extrabold text-forest">{vi.email.trustTitle}</p>
            <p className="mt-1 text-xs font-semibold leading-relaxed text-muted-brand">{vi.email.trustBody}</p>
            <p className="mt-2 text-xs font-medium text-muted-brand">{vi.email.privacyPrefix} · {vi.email.privacyLine}</p>
          </div>
          {canBypassEmail && (
            <button
              type="button"
