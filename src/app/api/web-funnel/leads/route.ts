...
export async function POST(request: NextRequest) {
  ...
  const upstream = await fetch(`${base}/v1/web-funnel/leads`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Lead-Access-Key': draftKey, 'X-Request-ID': requestId, 'X-Web-Funnel-BFF-Token': bffToken }, body: await request.text(), cache: 'no-store' });
